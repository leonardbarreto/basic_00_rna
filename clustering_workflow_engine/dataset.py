...
# # #     app()
# # # clustering_workflow_engine/dataset.py
# # from pathlib import Path
# # import pandas as pd
# # from loguru import logger
# # from tqdm import tqdm
...
# def save_dataset(X: pd.DataFrame, y: pd.Series, output_path: Path):
#     """Salva dataset processado como CSV"""
#     df_to_save = pd.concat([X, y], axis=1)
#     output_path.parent.mkdir(parents=True, exist_ok=True)
#     df_to_save.to_csv(output_path, index=False)
#     logger.info(f"Dataset salvo em {output_path}")
...
# @app.command()
# def main(dataset_name: str = "iris", output_path: Path = PROCESSED_DATA_DIR / "dataset.csv"):
#     """Processa e salva dataset"""
#     logger.info(f"Iniciando processamento do dataset '{dataset_name}'")
#     X, y = fetch_dataset(dataset_name)

#     # Exemplo de etapa intermediária (simula processamento)
#     for i in tqdm(range(10)):
#         if i == 5:
#             logger.info("Etapa intermediária concluída...")
...
def save_dataset(X: pd.DataFrame, y: pd.Series, dataset_name: str):
    """Salva dataset processado como CSV em PROCESSED_DATA_DIR"""
...
def main(dataset_name: str = "iris"):
    """Processa e salva dataset no PROCESSED_DATA_DIR"""
    logger.info(f"Iniciando processamento do dataset '{dataset_name}'")
    X, y = fetch_dataset(dataset_name)
